...
\begin{lstlisting}[language=C,frame=single,float=!ht,label={lst:green-led},caption={Static Mutex for a GPIO Output}]
    #[shared]
...
\begin{lstlisting}[language=C,frame=single,float=!ht,label={lst:populate-mutex},caption={Adding the Handler of the LED to the Mutex}]
...
\begin{lstlisting}[language=C,frame=single,float=!ht,label={lst:toggle-led},caption={A LED Toggle Function}]
    fn toggle_green_led() {
        free(|cs| {
...
Most of the peripherals on this board can be handled using driver functions provided by the \mycode{stm32h7xx-hal} crate. However the HSEM driver is not yet implemented in this crate, it is only part of the underlying \mycode{stm32h7} crate, which mostly only contains register definitions. Therefore part of this project is to write a minimal driver API for the hardware semaphores. It will contain methods for locking, both with the two-step and one-step methods, and unlocking a given semaphore.
